...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match std::env::args().nth(1).as_deref().unwrap_or("usb") {
        "usb" => {
            ...
            // discover → pick → connect. This sweep just takes the first device;
            // a real picker would let the user choose from the full list.
            let devices = rynk_serial::discover().await?;
            let device = devices
                .first()
                .ok_or_else(|| "no Rynk keyboard found over USB".to_string())?;
            if devices.len() > 1 {
                info!("{} keyboards found; connecting to {}", devices.len(), device.path);
            }
            run_all(device.connect().await?, false).await
        }
        "ble" => {
            info!("connecting over BLE GATT…");
            let devices = rynk_ble::discover().await?;
            let device = devices
                .first()
                .ok_or_else(|| "no Rynk keyboard found over BLE".to_string())?;
            if devices.len() > 1 {
                info!("{} keyboards found; connecting to {:?}", devices.len(), device.name);
            }
            run_all(device.connect().await?, true).await
        }
    ...
    if caps.max_morse > 0 {
        match client.get_morse(0).await {
            Ok(orig) => {
                ...
                ack(&mut fails, "restore morse 0", client.set_morse(0, orig).await);
            }
