...
///
/// Requests are cancel-safe only after the send completes: cancelling mid-send
/// can leave a partial frame and desync the device until reconnect.
...
impl<T: Read + Write> Client<T> {
    ...
    /// (cycling `1..=255`). A failed send leaves a partial frame that desyncs the
    /// device, so the link is marked dead. No flush: `write_all` hands over the
    /// whole frame and the shipped transports deliver on write.
