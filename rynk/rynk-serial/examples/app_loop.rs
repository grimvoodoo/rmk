...
use rynk_serial::SerialTransport;
...
/// Connect, retrying every second until a keyboard answers. With several
/// keyboards attached a real app lists `discover()` and lets the user pick; this
/// demo just takes the first discovered device.
async fn connect() -> Client<SerialTransport> {
    loop {
        match rynk_serial::discover().await {
            Ok(devices) if !devices.is_empty() => match devices[0].connect().await {
                Ok(client) => return client,
                Err(e) => warn!("connect failed ({e}); retrying in 1s"),
            },
