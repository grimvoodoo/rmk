...
/// device. The USB VID is firmware-customizable and a raw CDC port has no other
/// passive marker, so this fills the role the BLE service UUID plays over GATT.
///
/// Matched case-insensitively (Windows normalizes the serial in the device
/// instance id) and ASCII-only to stay valid there. It only narrows candidates;
/// the Rynk handshake remains the authoritative "is this a Rynk device" check.
