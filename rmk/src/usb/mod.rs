...
pub(crate) fn new_usb_builder<'d, D: Driver<'d>>(driver: D, keyboard_config: DeviceConfig<'d>) -> Builder<'d, D> {
    ...
    // The Rynk host library opens raw CDC ports, so it needs a marker to find
    // RMK keyboards without probing every serial device on the system. Prepend
    // an immutable magic to the USB serial number — independent of the
    // user-configured value, so a custom serial (or VID) can't hide the device.
    // The host matches on it, then confirms with the handshake.
    #[cfg(feature = "rynk")]
    let serial_number = {
